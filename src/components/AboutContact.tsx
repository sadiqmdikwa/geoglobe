  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await fetch(SCRIPT_URL, {
        method: 'POST',
        mode: 'no-cors', 
        // Headers removed to bypass CORS preflight blocks
                ...
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <input name="name" type="text" placeholder="Your Name" required className="w-full bg-black/40 border border-white/5 focus:border-geoCyan outline-none text-white p-4 rounded-2xl transition-all text-sm" />
                      <input name="email" type="email" placeholder="Email Address" required className="w-full bg-black/40 border border-white/5 focus:border-geoCyan outline-none text-white p-4 rounded-2xl transition-all text-sm" />
                    </div>
                    <select name="subject" className="w-full bg-black/40 border border-white/5 focus:border-geoCyan outline-none text-white p-4 rounded-2xl text-sm appearance-none cursor-pointer">

...
export default function GeoGame() {
  ...
  // 1. USE THE "GAME" TAB CSV LINK HERE 👇
  ...
  const prepareGame = () => {
    ...
    if (pool.length === 0) return alert("Insufficient data! Try another combination or add questions to the Game tab.");
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="main" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {["Mixed Global", "Flags", "Borders", "Anomalies"].map(cat => (
                <button key={cat} onClick={() => { setMainCat(cat); setStep('sub_cat'); }} className="p-8 bg-gray-900/50 border border-white/5 rounded-[2rem] text-left hover:border-geoCyan transition-all group backdrop-blur-md">
          ...
          <motion.div key="sub" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {["Global", "Africa", "Asia", "Europe", "Americas", "Oceania"].map(reg => (
                <button key={reg} onClick={() => { setSubCat(reg); setStep('settings'); }} className="p-6 bg-gray-900/50 border border-white/5 rounded-2xl hover:bg-geoCyan hover:text-black font-bold transition-all uppercase tracking-widest text-sm">
                  {reg}
                </button>
              ))}
            </div>
        ...
        {step === 'settings' && (
          <motion.div key="set" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-10">
            ...
            <button onClick={prepareGame} className="bg-geoCyan text-black px-16 py-6 rounded-3xl font-black text-xl hover:shadow-[0_0_30px_rgba(0,255,255,0.4)] transition-all flex items-center gap-3 mx-auto uppercase">
              Start Mission
            </button>
          </motion.div>
        )}
        ...
        {step === 'playing' && (
          <motion.div key="play" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-2xl mx-auto">
            ...
            <div className="bg-gray-900/80 border border-white/10 p-8 md:p-12 rounded-[3rem] shadow-2xl relative backdrop-blur-xl">
                {mainCat === "Flags" && (
                    <div className="w-full aspect-video flex items-center justify-center mb-8 bg-black/40 rounded-3xl overflow-hidden shadow-inner">
                      <img src={`https://flagcdn.com/w640/${shuffledQuestions[currentIndex].countryCode}.png`} className="h-32 md:h-44 object-contain" />
                    </div>
                )}
                ...
                <input ref={inputRef} value={userInput} onChange={e => setUserInput(e.target.value)} disabled={gameState === "revealed"} placeholder="IDENTIFY TARGET..." className="w-full p-6 bg-black/40 border-2 border-white/5 rounded-2xl text-white text-center text-xl font-black focus:border-geoCyan/50 outline-none mb-6 transition-all tracking-widest" onKeyDown={e => e.key === 'Enter' && handleCheck()} />
                ...
                <div className="flex gap-4">
                  <button onClick={handleCheck} className="flex-grow py-6 bg-geoCyan text-black font-black text-xl rounded-2xl hover:bg-white transition-all shadow-lg active:scale-95">
                      {gameState === "revealed" ? "NEXT OBJECTIVE" : "CONFIRM IDENTITY"}
                  </button>
                  {gameState === "playing" && (
                    <button onClick={() => { setIsCorrect(false); setGameState("revealed"); }} className="p-6 bg-white/5 text-gray-500 rounded-2xl hover:text-geoCyan hover:bg-white/10 transition-all border border-white/5" title="Reveal Answer">
                      <Eye size={24} />
                    </button>
                  )}
                </div>
